...
# some base images (e.g. Lambda Stack) ship system Pillow/jinja2 too old for current transformers
pip install -q --upgrade "pillow>=10" "jinja2>=3.1"

# the Dockerfile declares adapters/ a VOLUME, so docker pre-creates it empty;
# check for the config file itself, not just the directory
[ -f adapters/legal/adapter_config.json ] || python3 bake_loras.py
